...
def main_screen():
    ...
    root.title("MainPage")
    Label(text="MainPage", bg="grey", width="300", height="2", font=("Arial", 13)).pack()
    Label(text="").pack()
    Button(text="Login", height="2", width="30", command=Login).pack()
    Label(text="").pack()
    Button(text="Register", height="2", width="30", command=Register).pack()
    Label(text="").pack()
    Button(text="Exit", height="2", width="30", command=exitt).pack()
    root.mainloop()
...
def login_success():
            global root2
            root2 = Toplevel(root)
            root2.title("Login Success")
            root2.geometry("300x250")
            Label(root2, text="Welcome" + "names").pack()
            Button(root2, text="Logout", command=exitt).pack()
            root.mainloop()
...
def Register():
    ...
    path = os.getcwdb()
    faceFolder = '\\faces\\'
    # print(type(faceFolder))
    path = path + bytes(faceFolder, 'utf-8') + bytes(face_id, 'utf-8') + bytes('\\', 'utf-8')
    # print(path)
    os.mkdir(path)
    ...
    while True:
        ...
        open_eyes_glasses = open_eyes_detector.detectMultiScale(gray, 1.1, 5)
        for (ex, ey, ew, eh) in open_eyes_glasses:
            cv2.rectangle(img, (ex, ey), (ex + ew, ey + eh), (0, 255, 0), 2)

        right_eye = right_eye_detector.detectMultiScale(gray, 1.1, 5)
        for (ex, ey, ew, eh) in right_eye:
            right_face = img[y:y + h, x:x + int(w / 2)]
            cv2.rectangle(right_face, (ex, ey), (ex + ew, ey + eh), (255,255,255), 2)

        left_eye = left_eye_detector.detectMultiScale(gray,1.1,5)
        for (ex, ey, ew, eh) in left_eye:
            left_face = img[y:y + h, x + int(w / 2):x + w]
            cv2.rectangle(left_face, (ex, ey), (ex + ew, ey + eh), (255,255,255), 2)

            # Save the captured image into the datasets folder
            count += 1
            cv2.imwrite(str(path, 'utf-8') + str(face_id) + '.' + str(count) + ".jpg", gray[y:y + h, x:x + w])
...
def Login():
    def init():
        ...
        images = []
        for dir, _, files in tqdm(os.walk(dataset)):
            for file in files:
    ...
    if __name__ == "__main__":
        ...
        while True:
            frame = detect_and_display(model, video_capture, face_detector, open_eyes_detector, left_eye_detector,
                                       right_eye_detector, data, eyes_detected)
            cv2.imshow("Face Verification window", frame)
